...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((float64(runningCaloriesMeanSpeedMultiplier) * speed * runningCaloriesMeanSpeedShift) * weight / float64(mInKm) * duration * float64(minInH))
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speed, 2)/(height/float64(cmInM)))*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	...
	return (speed + swimmingCaloriesMeanSpeedShift) * float64(swimmingCaloriesWeightMultiplier) * weight * duration
